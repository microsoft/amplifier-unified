...
PATH entries and the current directory are excluded. Discovery does not import
Python packages, load Node packages, return environment variables or render files.
...
The worker's separately managed environment does not inherit this group.

Artifact workflows must still distinguish generation, rendering, visual review
and spreadsheet recalculation. All four validation fields start as `not_run`.
