...
async def resolve_defaults(home, workspace, bundle=None, app_bundle=None):
    ...
    chosen=bundle or defaults(home,workspace,app_bundle)['effective']
    ...
    try:
        output,_=await asyncio.wait_for(process.communicate(json.dumps({'home':str(home),'workspace':workspace,'bundle':chosen}).encode()),45)
        result=json.loads(output)
        if process.returncode or result.get('error'):raise ValueError('Draft default resolution failed')
        return {**result,'bundle':chosen}
    finally:
