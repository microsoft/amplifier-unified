...
class AppService:
    ...
    async def dispatch(self, action, args=None, origin="ui", command_id=None, expected_revision=None, *, include_state=True, caller_session_id=None):
        ...
        async with self.lock:
            ...
            if expected_revision is not None and expected_revision != self.state["revision"]:
                raise AppError("The app changed. Refresh its state and retry.", 409)
            if work_paused(self.state) and (action in {"question.answer","conversation.send","session.takeover","worker.spawn","worker.steer","worker.message","call.start","feedback.submit","feedback.comment","feedback.get"} or (action.startswith("smartTools.") and action not in {"smartTools.context","smartTools.result"})):
                raise AppError("An ecosystem update is activating. Please retry in a moment.", 409)
            if action in {"question.answer","conversation.send","worker.spawn","worker.steer","worker.message","call.start"}:
        ...
        for fn, values in pending:
            if action == "conversation.send" and fn == self._send or action == "message.edit" and fn == self._edit_current or action in {"question.answer", "worker.message"}:
