...
ACTION_DEFINITIONS = {
    ...
    "configuration.defaults": ("Resolve new-chat bundle and model without creating a conversation",schema({"workspace":string(4000),"bundle":string(4000)},["workspace"])),
    "providers.list": ("List provider connections and setup status without creating a conversation",schema({"sessionId":string(200),"workspace":string(4000)},[])),
    ...
    "providers.models": ("Browse cached provider models; refresh only this provider when requested",schema({"id":string(200),"sessionId":string(200),"workspace":string(4000),"refresh":{"type":"boolean"}},["id"])),
