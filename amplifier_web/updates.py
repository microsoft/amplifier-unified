...
def configured_sources(service):
    ...
    selections = {(state['settings']['workspace'], state['settings']['bundle'])}
    selections.update((s['workspace'], s['bundle']) for s in state['sessions'])
    selections.update((w['path'], None) for w in state.get('workspaces', []))
    try:
    ...
    for workspace, selected in selections:
        try:
            config = read_config(workspace, home=home)
