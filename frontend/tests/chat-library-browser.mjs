...
try{
 ...
 const waitScope=scope=>page.waitForFunction(scope=>window.amplifier.getState().chatNavigation?.scope?.mode===scope,scope);
 const waitPage=index=>page.waitForFunction(index=>window.amplifier.getState().chatNavigation?.index===index,index);
 const waitFilter=value=>page.waitForFunction(value=>window.amplifier.getState().chatNavigation?.scope?.filter===value,value);
 ...
 await page.waitForFunction(()=>window.amplifier.getState().chatNavigation.total===3);
 ...
 await page.waitForFunction(()=>window.amplifier.getState().chatNavigation.total===2);
 ...
 await page.waitForFunction(id=>!window.amplifier.getState().pinnedSessionIds.includes(id),quiet);
 ...
 await page.waitForFunction(id=>window.amplifier.getState().chatNavigation.items.find(chat=>chat.id===id)?.title==='Beta latest renamed',beta.id);
 ...
 await page.waitForFunction(id=>window.amplifier.getState().chatNavigation.items[0]?.id===id,quiet);
 ...
 await page.waitForFunction(id=>window.amplifier.getState().chatNavigation.items[0]?.id===id,beta.id);
 const agentScope=await api('/api/fixture/agent',{operation:'get_state',args:{path:'/chatNavigation/scope/mode'}});
 const agentFirst=await api('/api/fixture/agent',{operation:'get_state',args:{path:'/chatNavigation/items/0/id'}});
 assert.equal(agentScope.value,'all');assert.equal(agentFirst.value,beta.id);
