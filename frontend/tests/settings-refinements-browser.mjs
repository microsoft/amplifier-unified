...
try{
 ...
 await expect(root.getByText('Use GitHub CLI sign-in',{exact:true})).toBeVisible();await root.getByLabel(/Use GitHub CLI sign-in/).check();
 ...
 await expect(page.getByLabel('Speaking voice').locator('option')).toHaveCount(22);await page.getByLabel('Speaking voice').selectOption('willow');
 ...
 await page.getByLabel('Voice model',{exact:true}).selectOption('gpt-realtime-2.1');await expect(page.getByLabel('Speaking voice')).toHaveValue('marin');await expect(page.getByLabel('Speaking voice').locator('option')).toHaveCount(10);await page.getByLabel('Let me interrupt by speaking').click();await expect(page.getByLabel('Let me interrupt by speaking')).not.toBeChecked();
