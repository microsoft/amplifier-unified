...
try{
 ...
 await outputs.getByLabel('Output title',{exact:true}).fill('Saved page');await outputs.getByLabel('File in this workspace',{exact:true}).fill('page.png');await outputs.getByRole('button',{name:'Attach output',exact:true}).click();
 const picture=outputs.locator('.a-selection-card').filter({has:page.getByText('Saved page',{exact:true})}).first();await picture.getByRole('button',{name:'Read saved output'}).click();await detail.getByRole('button',{name:'View saved PNG'}).click();const preview=detail.getByRole('img',{name:'Saved output: Saved page'});await expect(preview).toBeVisible();assert.equal(await preview.evaluate(el=>el.naturalWidth),1);const imageUrl=await preview.getAttribute('src');assert.equal((await page.request.get(ready.url+imageUrl)).headers()['content-type'],'image/png');
 ...
 console.log(JSON.stringify({passed:true,writingCopyPreservesOriginal:true,immutableFileAndDownload:true,exactPngPreview:true,realGitReviewAndAnchoredNote:true,unlinkRestore:true,reload:true,mobileBounds:true,draftSelectionPreserved:true,noModelInvocations:true}));
