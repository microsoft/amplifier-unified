...
try{
 ...
 await openSettingsPage(page,'smart-tools');await page.getByRole('button',{name:'Browse catalog',exact:true}).click();
 ...
 for(const width of [320,390,736,1024]){await page.setViewportSize({width,height:900});await page.locator('#filter-smart-tool-catalog').fill('Research');if(width<960){await tools.locator('[data-collection-id=tool-1]>button').click();await expect(tools.getByRole('button',{name:'Back to catalog',exact:true})).toBeVisible();await tools.getByRole('button',{name:'Back to catalog',exact:true}).click();}assert.equal(await page.locator('.a-dialog').evaluate(el=>el.scrollWidth<=el.clientWidth),true,`overflow at ${width}`);}
