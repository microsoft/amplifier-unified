...
try {
 ...
 const sid=await page.evaluate(()=>window.amplifier.getState().selectedSessionId),target={sessionId:sid};
 ...
 await page.getByRole('button',{name:'Stop screen sharing',exact:true}).click();await expect.poll(async()=>(await inspect()).computerGrants[0]||null).toBe(null);
 assert.equal((await action('computer.visual.status',target)).result.available,false);
 ...
 await page.keyboard.press('Escape');await page.getByRole('button',{name:'End call',exact:true}).click();
 await expect(page.getByRole('status',{name:'Screen sharing active'})).toBeVisible();
 assert.equal(await page.evaluate(()=>window.captureTrack.readyState),'live');assert.equal((await inspect()).computerGrants.length,1);
 await page.getByRole('button',{name:'Stop sharing',exact:true}).click();await expect.poll(()=>page.evaluate(()=>window.captureTrack.readyState)).toBe('ended');assert.deepEqual(errors,[]);
 ...
 assert.deepEqual((await inspect()).nativeCalls,['status']);assert.equal((await inspect()).computerGrants[0]||null,null);
 await allow.click();await expect(page.getByRole('button',{name:'Capture screen',exact:true})).toBeEnabled();
 const grant=(await inspect()).computerGrants[0];assert.equal(grant.source.kind,'native-foreground');assert.equal(grant.source.host.identitySource,'foundation-owner-hostname');
 await new Promise(r=>setTimeout(r,2100));
 await page.getByRole('button',{name:'Capture screen',exact:true}).click();
