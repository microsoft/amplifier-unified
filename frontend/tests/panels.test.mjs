...
test('no pending updates still distinguishes failed checks from current sources',()=>{
 ...
 const html=renderToStaticMarkup(React.createElement(UpdateSettings,{state,act}));
 assert.match(html,/No updates available from the last check/);assert.match(html,/Needs attention/);assert.match(html,/Offline source/);assert.match(html,/a-check-result error/);
});
