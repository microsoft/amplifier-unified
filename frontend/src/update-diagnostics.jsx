...
const phaseLabels={'candidate-install':'Prepare application','candidate-probe':'Check candidate package','candidate-version':'Check candidate version','candidate-record':'Save candidate validation','target-discovery':'Find active installation','runtime-close':'Stop idle runtimes','recovery-record':'Save recovery reference','replacement-install':'Install application','replacement-probe':'Check installed package','replacement-version':'Check installed version','restart-configuration':'Prepare restart settings','restart-helper-file':'Prepare restart helper','restart-helper':'Start restart helper','service-restart':'Restart system service','service-restart-request':'Request service restart','restart-request':'Request restart','restart-readiness':'Check restarted host readiness','restart-ack':'Confirm restarted version','restart-reconcile':'Verify current installation','activation-validation':'Check staged release','ecosystem-copy':'Prepare ecosystem copy','ecosystem-source-preflight':'Check cached source changes','ecosystem-runtime-preflight':'Check installed worker sources','ecosystem-runtime-lock':'Resolve worker dependencies','ecosystem-runtime-freeze':'Record qualified worker','ecosystem-runtime-freeze-install':'Verify recorded worker','ecosystem-validation':'Validate ecosystem','ecosystem-fetch':'Fetch source update','ecosystem-checkout':'Switch staged source','ecosystem-probe':'Check bundles and modules','ecosystem-activation':'Activate ecosystem','ecosystem-stage':'Prepare ecosystem','stage':'Prepare application update'};
const recoveryGuidance={'module-prepare-failed':'A module could not be prepared in the new worker. The current ecosystem is preserved; review its dependencies before retrying.','bundle-not-found':'A configured bundle could not be found. Review its registration in Bundles & modules.','bundle-load-failed':'A configured bundle could not be loaded. Review its source and dependencies in Bundles & modules.','bundle-validation-failed':'A configured bundle did not pass validation. Review its configuration before retrying.','bundle-dependency-failed':'A bundle dependency could not be prepared. Review its source and dependency configuration.','protected-runtime-source':'Local source changes or an explicit dependency override were preserved. Review this module in Bundles & modules and preserve its source configuration before checking for updates again.','runtime-source-changed':'Worker sources changed while checking. Let source changes finish, then check for updates again.'};
...
export function diagnosticReceipt(diagnostics,reconciliation){
 ...
 const clean=event=>{
  ...
  if(event.probe)result.probe=Object.fromEntries(['ok','isolated','packageInEnvironment','frontendPresent','loginAvailable','standalone','providersPresent','cliAbsent','stage','errorType','version','pythonVersion','reason'].filter(key=>event.probe[key]!==undefined).map(key=>[key,event.probe[key]]));
  return result;
 };
