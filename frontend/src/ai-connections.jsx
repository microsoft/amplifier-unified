...
export function AIConnections({state,session,act,navigate}){
 ...
 const choose=s=>{if(sending.current)return;setKey('');setError('');setNotice('');edit({step:'connect',module:s.module,id:s.module.replace('provider-','')+'-'+crypto.randomUUID().slice(0,8),model:'',baseUrl:'',previousLoginId:null,scope:'global',saved:false,initializeRouting:providers.length===0});};
 const saveConnection=()=>run('providers.save',{id:d.id,module:d.module,config:{...selected?.config,...(service.auth==='optional-key'?{base_url:d.baseUrl?.trim()}: {})},scope:d.scope||'global',...(service.auth==='signin'||!key.trim()?{}:{apiKey:key})},service.auth==='signin'?'signin':'models');
   ...
   {step==='services'?<><h4>Which service would you like to use?</h4><div className="a-everyday-list">{aiServices.map(s=><SettingsLink key={s.module} disabled={busy} title={s.name} description={s.description} onClick={()=>choose(s)}/>)}</div><button className="a-link" data-action="view.update" onClick={advanced}>Other provider or custom endpoint</button></>:<>
    ...
    {step==='detail'?<><p>{selected?.credentialsConfigured?'Credentials saved on this Amplifier host.':'Review your account connection before choosing a model.'}</p><p>Model: <strong>{selected?.config?.default_model||selected?.config?.model||'Not selected'}</strong></p><div className="a-dialog-actions"><button className="a-primary" data-action="providers.models" disabled={busy} onClick={()=>{edit({step:'model'});run('providers.models',{id:d.id})}}>Choose model</button><button className="a-soft" data-action="view.update" disabled={busy} onClick={()=>edit({step:'connect',previousLoginId:login?.loginId})}>{service.auth==='signin'?'Sign in':'Update key'}</button><button className="a-soft" data-action="providers.test" disabled={busy} onClick={()=>run('providers.test',{id:d.id})}>Check connection</button></div><p className="a-caption">Checking verifies the model catalog, without sending a chat request.</p><button className="a-link" data-action="view.update" onClick={advanced}>Full connection settings</button></>:step==='connect'?<>
     {service.auth==='signin'?<>
      ...
      <label htmlFor="ai-private-key">{service.auth==='token'?'GitHub token':service.auth==='optional-key'?'API key (optional)':'API key'}</label><input id="ai-private-key" type="password" autoComplete="off" spellCheck={false} value={key} onChange={e=>setKey(e.target.value)} disabled={busy}/><p className="a-caption">{service.auth==='optional-key'?'Leave blank if your local service does not need a key.':'Stored privately on the Amplifier host.'}</p>
      <SettingsActions><button className="a-primary" data-action="providers.save" disabled={busy||(service.auth==='optional-key'?!/^https?:\/\/[^\s]+$/.test(d.baseUrl||''):!key.trim())} onClick={saveConnection}>{busy?'Connecting…':'Save and check connection'}</button></SettingsActions>
     </>}
    </>:step==='model'?<>
