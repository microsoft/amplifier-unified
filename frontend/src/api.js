...
export function visibleView(root, clientId) {
  ...
  return {clientId,panes,visibleText:root.innerText.slice(0,60000),controls,selectedText:window.getSelection()?.toString().slice(0,10000)||'',viewport:{width:innerWidth,height:innerHeight,scrollX,scrollY},url:location.pathname};
}
