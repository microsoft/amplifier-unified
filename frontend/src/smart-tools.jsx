...
export function SmartToolsSettings({state, act}) {
  ...
  return <section className="a-settings-section a-smart-tools" data-part="smart-tools-settings">
    ...
    {editor.page === 'connection' && <form id="smart-connection-form" onSubmit={async event => {event.preventDefault();try {const env = parseEnvironment(connection.env);await run('smartTools.configure',{...(connection.id?{id:connection.id}:{}),name:connection.name.trim(),command:connection.command.trim(),args:connection.args.split('\n').map(value => value.trim()).filter(Boolean),env,...(connection.cwd.trim()?{cwd:connection.cwd.trim()}:{})});} catch(caught) {setError(caught.message);}}}>
      ...
      <SettingsActions><button type="submit" form="smart-connection-form" className="a-primary" data-action="smartTools.configure" disabled={!connection.name.trim() || !connection.command.trim() || anyPending}><Check/>Save connection</button><button type="button" className="a-soft" data-action="view.update" onClick={() => navigate('home')}>View connections<ArrowRight/></button></SettingsActions>
    </form>}
    ...
    {editor.page === 'tool' && tool && <>
      ...
      <form id="smart-tool-form" onSubmit={event => {event.preventDefault();try {const args=collectArguments(toolSchema,editor.values,editor.raw,editor.json);run('smartTools.call',{id:server.id,name:tool.name,arguments:args});} catch(caught) {setError(caught.message);}}}>
