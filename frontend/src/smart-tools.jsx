...
export function SmartToolsSettings({state, act, simple=false, navigateSettings}) {
  ...
  const navigate = (page, patch={}) => {setError('');setLocal(null);edit({page,returnPage:page==='source'&&editor.page==='catalog'?'catalog':null,...patch});};
  ...
  useEffect(()=>{if(!simple||firstVisit.current)return;firstVisit.current=true;if(!state.view?.smartToolsEditor?.page){navigate(installations.length||servers.length?'home':'catalog');if(!catalog.length)run('smartTools.catalog');}},[simple]);
  const installSelected=()=>{edit({catalogReview:false});return run('smartTools.installBatch',{ids:selectedItems.map(item=>item.id),extrasById:Object.fromEntries(selectedItems.map(item=>[item.id,(editor.catalogExtras?.[item.id]||'').split(',').map(extra=>extra.trim()).filter(Boolean)]))});};
