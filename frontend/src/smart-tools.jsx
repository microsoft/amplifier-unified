...
export function SmartToolsSettings({state, act, simple=false, navigateSettings}) {
  ...
  const finishInstallation=item=>navigate(simple?'setup':'source',{repository:sourceOf(item),ref:item.ref||'',path:item.path||'',extras:(item.extras||[]).join(', ')});
  ...
  const catalogStage=simple&&(editor.catalogReview?'review':editor.catalogInstalling||batchBusy?'install':'browse');
  ...
  return <section className="a-settings-section a-smart-tools" data-part="smart-tools-settings" data-simple={simple}>
    ...
    {editor.page==='setup'&&<><h4>{installed?.name||installed?.manifest?.name||'Finish setup'}</h4><p>The package is installed. A connection and any required account access are separate.</p>{servers.filter(row=>row.installationId===installed?.id&&!!installed?.id).map(row=><button key={row.id} className="a-soft" data-action="view.update" onClick={()=>navigate('server',{serverId:row.id})}>Open {row.name}</button>)}{!servers.some(row=>row.installationId===installed?.id&&!!installed?.id)&&<div className="a-ai-hint"><strong>Advanced setup required</strong><p>This tool does not provide a guided connection setup. Use its documented connection details before using it.</p><button className="a-soft" data-action="view.update" onClick={()=>{navigate('source');navigateSettings?.('tool-connections')}}>Open setup instructions</button></div>}<details className="a-everyday-disclosure"><summary>Installation details</summary><div><p>{installed?.description}</p><code className="a-wrap">{editor.repository}</code><p className="a-caption">{installed?.nextStep}</p></div></details></>}
    {editor.page === 'source' && <>
      <h4>{simple?'Add from GitHub URL':'Add from Git'}</h4><p>{simple?'Paste the repository link for a Smart Tool. Review its details before installing.':'Inspect the tool’s manifest and source revision, then install the Python package into its own environment.'}</p>
