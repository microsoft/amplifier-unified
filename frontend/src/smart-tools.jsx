...
export function SmartToolsSettings({state, act}) {
  ...
  return <section className="a-settings-section a-smart-tools" data-part="smart-tools-settings">
    ...
    {editor.page === 'catalog' && <ActivityRegion name="smart-tool-catalog" busy={working(operation('smartTools.catalog'))}>
      <div className="a-smart-heading"><h4>Smart Tools catalog</h4><button type="button" className="a-icon" aria-label="Refresh Smart Tools catalog" aria-busy={working(operation('smartTools.catalog'))||undefined} data-action="smartTools.catalog" disabled={working(operation('smartTools.catalog'))} onClick={() => run('smartTools.catalog')}><RefreshCw/></button></div>
    ...
    {editor.page === 'source' && <>
      ...
      <div className="a-dialog-actions"><button type="button" className="a-primary" aria-busy={working(installationOp)||undefined} data-action="smartTools.install" disabled={!editor.repository.trim() || anyPending} onClick={() => run('smartTools.install',{...sourceArgs(),...(editor.extras.trim() ? {extras:editor.extras.split(',').map(value => value.trim()).filter(Boolean)} : {})})}><Download/>{installed ? 'Install source revision' : 'Install package'}</button></div><OperationNotice operation={installationOp} busyMessage="Installing the package in its own environment…"/>
    ...
    {editor.page === 'tool' && tool && <>
      ...
      <form onSubmit={event => {event.preventDefault();try {const args=collectArguments(toolSchema,editor.values,editor.raw,editor.json);run('smartTools.call',{id:server.id,name:tool.name,arguments:args});} catch(caught) {setError(caught.message);}}}>
        ...
        <div className="a-dialog-actions"><button className="a-primary" aria-busy={working(callOperation)||undefined} data-action="smartTools.call" disabled={anyPending || !['connected','ready'].includes(server.status)}><Play/>Run tool</button></div>
