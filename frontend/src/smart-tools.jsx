...
export function SmartToolsSettings({state, act, simple=false, navigateSettings}) {
  ...
  return <section className="a-settings-section a-smart-tools" data-part="smart-tools-settings" data-simple={simple}>
    ...
    {editor.page === 'catalog' && <ActivityRegion className={simple?"a-tool-catalog-simple":undefined} name="smart-tool-catalog" busy={working(operation('smartTools.catalog'))}>
      <div className="a-catalog-controls"><div className="a-smart-heading"><h4>Smart Tools catalog</h4><button type="button" className="a-icon" aria-label="Refresh Smart Tools catalog" data-action="smartTools.catalog" disabled={working(operation('smartTools.catalog'))} onClick={()=>run('smartTools.catalog')}><RefreshCw/></button></div>
      ...
      {(selectedItems.length>0||batchBusy)&&<SettingsActions><span className="a-catalog-selection-summary">{selectedItems.length} selected{selectedItems.some(item=>!pageItems.includes(item))&&<small>{selectedItems.filter(item=>!pageItems.includes(item)).length} outside this view</small>}</span><button className="a-primary" disabled={!selectedItems.length||batchBusy} data-action="smartTools.installBatch" onClick={()=>simple?edit({catalogReview:true}):installSelected()}><Download/>{simple?'Review installation':'Install selected'} ({selectedItems.length})</button></SettingsActions>}
