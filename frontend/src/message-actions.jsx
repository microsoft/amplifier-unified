...
export function MessageEntry({message:m,session,state,act,stamp,working,forkTurn,retry,dispatch=act}){
 ...
 return <article className={`a-message a-${m.role==='user'?'user':'assistant'}`} data-message-id={m.id}>
  ...
  </form>:<DetailText text={m.text|| (working?'…':'')} reference={m.textDetail} markdown={m.role!=='user'} automatic={m.role==='assistant'} writingContext={m.role==='assistant'?{sessionId:session.id,messageId:m.id,act}:undefined}/>}
