...
export function MessageEntry({message:m,session,state,act,stamp,working,forkTurn}){
 const [saving,setSaving]=useState(false),[detailError,setDetailError]=useState(''),edit=state.view?.messageEdit,editing=edit?.sessionId===session.id&&edit?.messageId===m.id;
 ...
 return <article className={`a-message a-${m.role==='user'?'user':'assistant'}`} data-message-id={m.id}>
  ...
  {!editing&&<div className="a-message-actions">
   ...
   {m.role==='user'&&<button type="button" className="a-icon" title={session.historyReadOnlyReason|| (session.workspaceAvailable===false?'Workspace folder unavailable':blocked?'Wait for the current work to finish':'Edit message')} aria-label="Edit message" disabled={blocked||saving} data-action="view.update" onClick={async()=>{setSaving(true);try{patch({sessionId:session.id,messageId:m.id,text:m.textDetail?await readDetail(m.textDetail):m.text})}catch(e){setDetailError(e.message)}finally{setSaving(false)}}}><Pencil/></button>}
