...
export function BundleControl({state,session,act,working}){
 ...
 const disabled=working||pending||unavailable;
 async function run(action){
  if(submitting.current)return;submitting.current=true;
  try{await act(action,action==='session.create'?{bundle:draft.bundle}:{sessionId:session.id,bundle:draft.bundle,...(action!=='bundle.preview'?{previewId:preview.previewId,resetModel:!!draft.resetModel}:{})})}
  finally{submitting.current=false}
 }
 return <div className="a-model-control a-bundle-control"><button type="button" className="a-model-trigger a-bundle-trigger" aria-label="Conversation bundle" aria-expanded={!!open} data-action="view.update" onClick={()=>edit({open:!open,sessionId:session?.id||null,bundle:current,resetModel:false})}><Layers/><span>{bundleLabel(state,current)}</span><ChevronDown/></button>{open&&<section className="a-model-popover a-bundle-popover" aria-label="Choose conversation bundle"><div className="a-settings-row"><strong>Conversation bundle</strong><button type="button" className="a-icon" aria-label="Close bundle settings" data-action="view.update" onClick={()=>edit({open:false})}><X/></button></div><p>Choose the tools, agents, and instructions for this conversation.</p><BundlePicker id="conversation-bundle" value={draft.bundle||current} onChange={bundle=>edit({bundle,resetModel:false})} state={state} act={act}/>
 {session?<><p className="a-caption">Switching keeps conversation history and compatible model choices. Previous module edits, modes, and budget overrides are reset to the new bundle.</p><button type="button" className="a-soft" disabled={disabled||!draft.bundle?.trim()} data-action="bundle.preview" onClick={()=>run('bundle.preview')}>Preview changes</button>
 {ready&&<div className="a-bundle-changes" aria-label="Bundle changes">{Object.entries(preview.changes||{}).map(([kind,change])=><div key={kind}><strong>{kind[0].toUpperCase()+kind.slice(1)}</strong>{'after'in change?<p>{change.before||'None'} → {change.after||'None'}</p>:<>{change.added?.length>0&&<p>Add: {change.added.join(', ')}</p>}{change.removed?.length>0&&<p>Remove: {change.removed.join(', ')}</p>}{!change.added?.length&&!change.removed?.length&&<p>Same set; settings may change.</p>}</>}</div>)}<p>Instructions are replaced. Enabled app capabilities and shared settings still apply.</p>{preview.appCapabilities?.length>0&&<details><summary>Added app capabilities ({preview.appCapabilities.length})</summary><ul>{preview.appCapabilities.map(value=><li key={value} className="a-wrap">{value}</li>)}</ul></details>}{!preview.modelCompatible&&<><p>Your pinned model is unavailable in this bundle.</p><label><input type="checkbox" checked={!!draft.resetModel} data-action="view.update" onChange={e=>edit({resetModel:e.target.checked})}/> Use the new bundle’s model</label></>}<div className="a-dialog-actions"><button type="button" className="a-primary" disabled={disabled||(!preview.modelCompatible&&!draft.resetModel)||draft.bundle===current} data-action="bundle.switch" onClick={()=>run('bundle.switch')}>Switch bundle</button><button type="button" className="a-soft" disabled={disabled||(!preview.modelCompatible&&!draft.resetModel)} data-action="bundle.fork" onClick={()=>run('bundle.fork')}>Fork with this bundle</button></div></div>}
 {working&&<p>Finish this turn and its workers to preview or switch bundles.</p>}{operation&&<ResultNotice phase={operation.phase} message={operation.error||(pending?'Preparing bundle configuration…':operation.phase==='ready'?operation.action==='bundle.preview'?'Preview ready':operation.action==='bundle.fork'?'Fork created':'Bundle switched':'')}/>}</>:<button type="button" className="a-primary" disabled={!draft.bundle?.trim()} data-action="session.create" onClick={()=>run('session.create')}>Start with this bundle</button>}</section>}</div>;
}
