...
export function AppearanceSettings({state,shell,act,name,css,preview,setName,setCss,setPreview,defaultSkin}){
 ...
 return <div className="a-appearance-settings">
  <div className="a-appearance-intro"><div><p>Make this space yours.</p><small>Choose an appearance to try it. Apply when it feels right.</small></div><span className="a-appearance-current"><Palette size={14}/>{preview?'Previewing '+name:state.theme?.name||'Amplifier'}</span></div>
  <div className="a-appearance-gallery" aria-label="Appearance library">
   {library.items.map(item=><button type="button" key={item.id} className="a-appearance-card" aria-label={'Preview '+item.name} aria-pressed={selected===item.id} disabled={busy} data-action="theme.preview" onClick={()=>choose(item)}>
    <Swatch item={item}/><div className="a-appearance-card-caption"><div><strong>{item.name}</strong><small>{item.tag}</small></div>{selected===item.id?<Check/>:<Palette/>}</div><p>{item.description}</p>
   </button>)}
   {!library.items.length&&!libraryError&&<p role="status">Loading appearances…</p>}
  </div>
  {preview&&<div className="a-appearance-preview" role="status"><span>Trying <strong>{name}</strong></span><div><button type="button" className="a-soft" disabled={busy} onClick={revert}>Cancel preview</button><button type="button" className="a-primary" disabled={busy} data-action="theme.apply" onClick={apply}><Check/>Use {name}</button></div></div>}
