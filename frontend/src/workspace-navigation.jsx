...
export function SimpleNavigation({model,workspaceHost,ChatDetails}){
 ...
 const [creating,setCreating]=useState(false),[error,setError]=useState(''),[extra,setExtra]=useState({items:[]}),[loadingMore,setLoadingMore]=useState(false);
 const patch=value=>act('view.update',{patch:value});
 const home=state.homeNavigation||chats,workspaces=[...new Map([...extra.items,...(state.workspaceOverview?.items||state.workspaces||[])].map(row=>[row.id,row])).values()].sort((a,b)=>a.name.localeCompare(b.name));
 const workspacePaths=new Set(workspaces.map(row=>row.path)),nextOffset=extra.nextOffset===undefined?state.workspaceOverview?.nextOffset:extra.nextOffset;
 const pins=(home.items||[]).filter(row=>row.pinned),recent=(home.items||[]).filter(row=>!row.pinned&&(row.location?.kind==='managed'||!row.workspaceId&&!workspacePaths.has(row.workspace))).slice(0,12);
 ...
 return <div className="a-simple-navigation">
  ...
  <section aria-label="Workspaces"><div className="a-simple-nav-heading"><h4 className="a-nav-chat-group-title">Workspaces</h4><button type="button" className="a-icon" ref={createButton} aria-label="New workspace" onClick={()=>setCreating(true)}><Plus/></button></div>
   ...
   {nextOffset!=null&&<button type="button" className="a-link" disabled={loadingMore} onClick={moreWorkspaces}>{loadingMore?'Loading…':'More workspaces'}</button>}
  </section>
