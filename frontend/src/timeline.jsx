...
function ExecutionNode({node,depth=0,ancestors=[],tree,act,now,expanded,toggle}){
 const tool=node.kind==='tool',open=expanded.has(node.id),requestOpen=expanded.has(`request:${node.id}`);
 ...
 return <div className="a-execution-node" data-kind={node.kind} data-node-id={node.id} data-action-kind={action?.kind}>
  <button type="button" className="a-execution-line a-execution-action-line" data-action="view.update" aria-expanded={open} onClick={()=>toggle(node.id)}>
   <ChevronRight className={`a-execution-chevron ${open?'open':''}`}/><Icon/>
