...
export function ExecutionBlock({label,text,meta,children,copy=true,loading=false}){
 ...
 return <section className="a-execution-payload"><header><span>{label}</span>{meta&&<span>{meta}</span>}{copy&&<CopyText text={textValue(text)} label={label} disabled={loading}/>}</header>{children||<ExecutionText text={text}/>}</section>;
}
