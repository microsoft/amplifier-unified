...
export function LiveChatActivity({voice={},callActive,browsing,working,session,act,onReturn,children}){
 return <div className="a-work-live-controls">
  {callActive&&<div className="a-call-strip" data-part="voice"><AudioLines/><span>{voice.model||'Voice'} · {voice.status}</span><button className="a-icon" aria-label={voice.muted?'Unmute microphone':'Mute microphone'} data-action="call.mute" onClick={()=>act('call.mute',{muted:!voice.muted})}>{voice.muted?<MicOff/>:<Mic/>}</button><button className="a-soft" data-action="call.end" onClick={()=>act('call.end')}>End call</button><button className="a-link" data-action="call.keepAwake" aria-pressed={voice.keepAwake!==false} onClick={()=>act('call.keepAwake',{enabled:voice.keepAwake===false})}>{voice.wakeLock==='active'?'Screen staying awake':'Keep screen awake'}</button>{voice.keepAwake!==false&&['unsupported','unavailable','released'].includes(voice.wakeLock)&&<small role="status">Screen may lock. Keep this page visible; battery-saving settings can prevent staying awake.</small>}{voice.fallbackReason&&<small>{voice.fallbackReason}</small>}</div>}
  {children}
