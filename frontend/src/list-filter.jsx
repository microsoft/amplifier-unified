...
export function useListFilter(state,act,id,items,fields,label){
 ...
 const control=<div className="a-list-filter"><label htmlFor={'filter-'+id}><Search/>{label}</label><div className="a-inline-form"><input id={'filter-'+id} type="search" value={query} placeholder="Search or pattern, e.g. *openai*" data-action="view.update" onChange={e=>change(e.target.value)}/>{query&&<button type="button" className="a-icon" aria-label={'Clear '+label.toLowerCase()} data-action="view.update" onClick={()=>change('')}><X/></button>}</div><small role="status">{rows.length} of {items.length}{query&&!rows.length?' · No matches':''} · Text search or wildcards: * ? [abc] [!abc]</small></div>;
