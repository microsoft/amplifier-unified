...
export function updateOverview(updates={},options={}){
 ...
 const blocked=updates.pendingReplacement!=null||!!updates.pendingRestart||!!(seq.stage==='included'&&seq.included?.issues);
 ...
 const result={stage,installable,blocked,busy:active||updates.pendingReplacement!=null,continuing:!!seq.install,title:'Check for updates',detail:'We’ll check Amplifier and the components it uses.',tone:'neutral'};
 ...
 if(updates.pendingApp||updates.pendingRelease)return {...result,title:'Waiting for your work to finish',detail:'The update is ready. It will continue automatically when conversations, tools, and calls are idle.',tone:'working'};
 if(active)return {...result,title:updates.phase==='checking'?'Checking for updates':'Updating Amplifier',detail:updates.phase==='checking'?(stage==='application'?'Checking for a new app version first.':'Checking the components used by this version of Amplifier.'):'We’ll handle the app and its components in order. There’s nothing else to click.',tone:'working'};
 if(seq.nextStage||seq.install&&installable)return {...result,title:'Updating Amplifier',detail:'Continuing with the remaining components automatically.',tone:'working'};
 if(installable)return {...result,title:'Updates available',detail:(appAvailable?'A new app version is ready. ':'')+(components?`${components} component ${components===1?'update is':'updates are'} ${appAvailable?'also ':''}ready. `:'')+(options.autoInstall?'They’ll install automatically when work is idle.':'One update request handles the app and all eligible components.'),tone:'available'};
