...
export function ConversationLibrary({state,session,act}){
 ...
 return <section className="a-settings-section"><h3>Conversation library</h3>
  ...
  <form onSubmit={async e=>{e.preventDefault();if(await run('collection.create',{name}))setName('')}}><label htmlFor="new-chat-collection">New collection name</label><input id="new-chat-collection" value={name} maxLength={100} onChange={e=>setName(e.target.value)}/><button type="submit" className="a-soft" data-action="collection.create" disabled={busy||!name.trim()}><Plus/>Create collection</button></form>
