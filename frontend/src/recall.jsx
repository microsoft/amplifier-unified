...
export function RecallSettings({session,act}){
 ...
 return <>
  <section className="a-settings-section" aria-label="Automatic memory"><h3>Automatic memory</h3><p>Workspace references can capture your stated preferences, settled decisions, and approaches you said worked. Review their exact sources and correct them when needed.</p>
   {personalization&&<>
    <label><input type="checkbox" checked={personalization.settings.contribute} disabled={busy} data-action="memory.configure" onChange={e=>configure({contribute:e.target.checked})}/>Contribute memory from idle conversations in this workspace</label>
    <p>Contribution uses each source conversation’s selected model and may incur provider charges. Up to {personalization.settings.maxCallsPerDay} attempts per day, 16,000 source characters and 4,096 output tokens per attempt. Only attributable user statements qualify; generated wording can be mistaken.</p>
    <label><input type="checkbox" checked={personalization.settings.use} disabled={busy} data-action="memory.configure" onChange={e=>configure({use:e.target.checked})}/>Use relevant saved memory automatically in this workspace</label>
    ...
    <label htmlFor="memory-call-limit">Maximum model attempts per day</label><input id="memory-call-limit" type="number" min={1} max={10} disabled={busy} value={personalization.settings.maxCallsPerDay} onChange={e=>{const n=Number(e.target.value);if(n>=1&&n<=10)configure({maxCallsPerDay:n})}}/>
    ...
    {personalization.lastContext&&<div aria-label="Last memory context"><strong>Last context supplied</strong>{personalization.lastContext.items.length?personalization.lastContext.items.map(row=><p key={row.id}>{row.id} · revision {row.revision} · matched {row.matchTerms.join(', ')}</p>):<p>No relevant memory was selected.</p>}</div>}
   </>}
