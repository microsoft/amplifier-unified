...
export function WorkspaceRail({state,session,act,selectSession,newSession,shell,footer}){
 ...
 <aside ref={nav} id="workspace-navigation" role={layout.narrow?'dialog':undefined} aria-modal={layout.narrow&&expanded||undefined} hidden={layout.narrow&&!expanded} inert={layout.narrow&&!expanded} className={`a-nav-slot ${pinned?'is-pinned':''} ${expanded?'is-expanded':''}`} data-docked={layout.docked} data-part="navigation" aria-label="Workspaces and conversations" onClick={e=>{if(layout.narrow&&e.target.closest('[data-navigation-select],[data-action="session.select"]'))close()}} onPointerEnter={e=>{if(!layout.narrow&&e.pointerType!=='touch')expand(true)}} onPointerLeave={e=>{if(!layout.narrow&&e.pointerType!=='touch'&&!e.currentTarget.contains(document.activeElement))expand(false)}}>
  <div className="a-nav-rail">
   ...
   <div className="a-nav-content a-nav-reveal"><ShellModules shell={shell}/></div>{footer}
  </div>
...
export function AgentCanvas({state,act,dispatch=act,suppressed=false}){
 ...
 return <CanvasControlsHost.Provider value={controlsHost}><aside ref={panel} id="workspace-canvas" className="a-canvas-panel" hidden={suppressed||!canvas.open||!state.selectedSessionId} inert={suppressed||!canvas.open||!state.selectedSessionId} aria-hidden={suppressed||!canvas.open||!state.selectedSessionId||undefined} data-part="canvas" data-focused={focused} data-controls={controls} data-pinned={!!view.canvasControlsPinned} aria-label="Agent canvas" role={focused?'dialog':undefined} aria-modal={focused||undefined}>
  ...
  <div className="a-canvas-chrome">
   ...
   <div className="a-canvas-controls" inert={!controls}>
    {(hasContent||hasArtifacts)&&<details className="a-file-actions-menu"><summary>Open…</summary><div className="a-canvas-actions"><ShellSlot name="canvas.toolbar"><button type="button" className="a-soft" aria-label={`Saved artifacts (${chatArtifacts(state).length})`} aria-pressed={!!draft.library} data-action="view.update" onClick={()=>changeDraft({library:!draft.library,open:false,browser:false})}><Library/>Saved outputs</button><button type="button" className="a-soft" aria-label="Open a website in canvas" aria-expanded={!!draft.browser} data-action="view.update" onClick={()=>changeDraft({browser:!draft.browser,open:false,library:false})}><Globe/>Website</button><button type="button" className="a-soft" aria-label="Open a file in canvas" aria-expanded={!!draft.open} data-action="view.update" onClick={()=>changeDraft({open:!draft.open,browser:false,library:false})}><FolderOpen/>File</button></ShellSlot></div></details>}
