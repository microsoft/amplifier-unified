...
export function AgentCanvas({state,act}){
 ...
 return <aside className="a-canvas-panel" data-part="canvas" style={{'--canvas-width':`${width}px`}} aria-label="Agent canvas">
  ...
  {draft.open&&<form className="a-canvas-file-form" onSubmit={e=>{e.preventDefault();act('canvas.show',{kind:draft.kind||'text',path:draft.path||''})}}><label htmlFor="canvas-file-path">File in this workspace</label><PathField id="canvas-file-path" value={draft.path||''} onChange={path=>changeDraft({path})} state={state} act={act} placeholder="README.md or a full path"/><div className="a-canvas-file-actions"><select aria-label="Canvas file format" value={draft.kind||'text'} data-action="view.update" onChange={e=>changeDraft({kind:e.target.value})}><option value="text">Text</option><option value="markdown">Markdown</option><option value="code">Code</option><option value="image">Image</option></select><button type="submit" className="a-primary" data-action="canvas.show" disabled={!draft.path?.trim()}>Open file<ChevronRight/></button></div></form>}
  {canvas.path&&<div className="a-canvas-file-path" title={canvas.path}>{canvas.path}</div>}
  <div className="a-canvas-body">{canvas.kind==='a2ui'?<A2UISurface surface={canvas.surface} act={act}/>:canvas.kind==='image'?<img className="a-canvas-image" src={canvas.content} alt={canvas.title||'Workspace image'}/>:canvas.kind==='markdown'?<Markdown text={canvas.content}/>:canvas.content?<pre className={canvas.kind==='code'?'a-canvas-code':'a-canvas-plain'}>{canvas.content}</pre>:<div className="a-canvas-empty"><PanelRight/><h2>A little more room to work</h2><p>Preview a workspace file here, or ask your agent to display a document or interactive view.</p><button type="button" className="a-soft" data-action="view.update" onClick={()=>changeDraft({open:true})}><FolderOpen/>Open a file</button></div>}</div>
