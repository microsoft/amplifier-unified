...
function SettingsOverview({state,session,navigate}){
 ...
 const cards=[['providers','models','Models & routing',`${providers.length} saved connections${state.setup?.active?' · '+state.setup.active+' routing':''}`],['app-bundles','bundles','Bundles & modules',`${entries.length} configured entries · defaults and conversation modules`],['smart-tools','smart-tools','Smart Tools',`${servers.length} connections · discover more in the catalog`],['updates','updates','Updates','App releases, ecosystem sources, and automatic updates']];
