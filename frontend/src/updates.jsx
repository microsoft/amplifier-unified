...
export function UpdateSettings({state,act}){
 ...
 const countUnknown=status=>unknown.filter(item=>item.status===status).length;
 ...
 return <section className="a-updates" data-part="updates">
  ...
  {!!unknown.length&&<div><h4>Other cached sources / usage unverified</h4><p className="a-caption">{unknown.length} sources: {countUnknown('check_failed')} check failed, {countUnknown('local_changes')} local changes, {countUnknown('update')} updates, {countUnknown('pinned')} pinned, {countUnknown('current')} current, {countUnknown('not_checked')} not checked.</p><p className="a-caption">These may be transitive dependencies or old caches, not proven unused. Checks and eligible updates continue normally. All copies are retained.</p>{!!unknownIssues.length&&<SourceList items={unknownIssues} state={state} act={act} id="unknown-source-issues"/>}</div>}
