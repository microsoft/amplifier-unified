...
export function workspaceChats(sessions=[],workspace){
 return sessions.filter(chat=>!workspace||(chat.workspaceId?chat.workspaceId===workspace.id:!!workspace.path&&chat.workspace===workspace.path));
}
...
export function headerChatChoices(state){
 ...
 if(selected&&!items.some(row=>row.id===selected.id))items.splice(Math.max(0,CHAT_PAGE_SIZE-1),1,selected);
