...
import {BundleControl,BundleDefaults} from './bundle-controls';
import {AttentionBadge,ActivityPanel,useReadCompletion} from './attention';
import {SmartToolsSettings} from './smart-tools';
import {SettingsGroup,FileDrop,PathField,BundlePicker} from './settings-ui';
...
function App(){
 ...
 async function send(e){
  ...
  try{
   const current=await ensureSession();setSending({sessionId:current.id,id,text:submittedText});
   await dispatch('view.update',{sessionId:current.id,patch:{draft:submittedText}});
   await dispatch('conversation.send',{sessionId:current.id,text:submittedText,attachmentIds:(current.draftAttachments||[]).map(file=>file.id),via:mode==='text'?'text':'chat'},{id});
   pendingView.current.settle(submittedToken);if(stagedDraft.current===submittedToken)stagedDraft.current=null;if(latest.current)setState(pendingView.current.apply(latest.current));
   if(latest.current.selectedSessionId===current.id&&lastDraft.current===submittedText){pendingView.current.settle(stagedDraft.current);stagedDraft.current=null;setDraft('');lastDraft.current='';await dispatch('view.update',{sessionId:current.id,patch:{draft:''}})}
  }catch(e){setError(e.code==='session_busy'?actionErrorMessage(e):'The send was not acknowledged. Your draft is kept; check the conversation before sending again. '+actionErrorMessage(e))}finally{setSending(null)}
 }
 ...
 const sendingHere=!!sending&&sending.sessionId===session?.id;
 ...
 return <div id="amp-one" className="a-chat-shell" ref={root} data-layout={presentation.layout||view.layout||'balanced'} data-density={presentation.density||'comfortable'} style={{colorScheme:scheme,...(presentation.accent?{'--a-accent':presentation.accent}:{})}} data-part="app">
  ...
  {ownership.blocked&&<div className="a-alert a-ownership" role="status"><span><strong>{ownership.label}.</strong> {ownership.detail}</span>{ownership.canTakeover&&<button className="a-soft" data-action="session.takeover" onClick={()=>act('session.takeover',{id:session.id})}>Continue here</button>}</div>}
  ...
  {panel&&<div className={`a-overlay ${panel==='settings'?'a-settings-overlay':''}`} data-part="overlay"><section role="dialog" aria-modal="true" aria-labelledby="panel-title" className={`a-dialog ${panel==='appearance'||panel==='agent'||panel==='runtime'||panel==='settings'?'wide':''}`} data-part="dialog"><div className="a-dialog-head"><h2 id="panel-title">{{'new-session':'Start a conversation',appearance:'Make it feel like you',agent:'What the agent sees',settings:'Your Amplifier',activity:'Ready for you',feedback:'Send feedback',worker:'Give it a worker lane','delete-session':'Remove chat?',runtime:'Session controls','session-details':'This conversation','subagent-history':'Subagent history'}[panel]||panel}</h2><button className="a-icon" data-action="view.update" aria-label="Close panel" onClick={close}><X/></button></div>
   ...
   {panel==='delete-session'&&<><p>Remove {session?.title} from this list? Its shared history stays on disk. Any work in progress in this app will stop.</p><div className="a-dialog-actions"><button className="a-primary" data-action="session.delete" onClick={async()=>{await act('session.delete',{id:session.id});close()}}>Remove chat</button><button className="a-soft" data-action="view.update" onClick={close}>Keep conversation</button></div></>}{panel==='settings'&&<><SettingsNavigation state={state} act={act}/>{view.settingsSection!=='capabilities'&&state.management?.error&&<div className="a-alert" role="alert"><span>{state.management.error}</span></div>}{view.settingsSection!=='capabilities'&&state.management?.phase==='working'&&<p className="a-management-status" role="status"><Loader className="a-progress-spinner"/>Updating your configuration…</p>}{(view.settingsSection||'setup')==='setup'&&<><p className="a-settings-overview">Your Python service runs locally and loads real Amplifier bundles. Provider credentials stay on the server.</p><dl className="a-meta-grid a-settings-overview"><div><dt>Runtime</dt><dd>{state.runtime?.available?'Available':state.runtime?.error||'Not installed'}</dd></div><div><dt>Session status</dt><dd>{session?.status||'No active conversation'}</dd></div></dl><SettingsGroup id="loaded-modules" title="Loaded session modules" summary="Toggle modules and edit this conversation’s running mount plan" state={state} act={act}><ModuleSettings state={state} session={session} act={act}/></SettingsGroup><SettingsGroup id="providers" title="Model providers" summary="Connections, credentials, models and provider options" state={state} act={act}><ProviderSettings state={state} session={session} act={act}/></SettingsGroup><SettingsGroup id="routing" title="Model routing" summary={state.setup?.active?`Active profile: ${state.setup.active}`:'Choose and customize models for each role'} state={state} act={act}><RoutingSettings state={state} act={act}/></SettingsGroup><SettingsGroup id="install-app" title="Install Amplifier" summary="A dedicated window and an icon in your Dock or home screen" state={state} act={act}><p>In Edge or Chrome, choose the install icon in the address bar or Install this site as an app in the browser menu. In Safari on Mac, use File → Add to Dock. On iPhone or iPad, use Share → Add to Home Screen.</p><p>Keep the Python service running on the host computer. Installation gives this interface its own window; the service still runs your conversations and tools.</p><p className="a-caption">Use localhost on this computer, or a trusted HTTPS address on other devices. Your saved chats and canvas artifacts stay on the host.</p></SettingsGroup><SettingsGroup id="defaults" title="Conversation defaults" summary="Bundle and voice models" state={state} act={act}><BundleDefaults state={state} act={act}/><label htmlFor="preferred-voice">Preferred voice model</label><select id="preferred-voice" data-action="settings.update" value={state.settings?.preferredVoice||'gpt-live-1'} onChange={e=>act('settings.update',{patch:{preferredVoice:e.target.value}})}><option value="gpt-live-1">GPT Live 1</option><option value="gpt-realtime-2.1">GPT Realtime 2.1</option></select><label htmlFor="fallback-voice">Fallback voice model</label><select id="fallback-voice" data-action="settings.update" value={state.settings?.fallbackVoice||'gpt-realtime-2.1'} onChange={e=>act('settings.update',{patch:{fallbackVoice:e.target.value}})}><option value="gpt-realtime-2.1">GPT Realtime 2.1</option><option value="gpt-live-1">GPT Live 1</option></select></SettingsGroup>{session&&<SettingsGroup id="conversation" title="Current conversation" summary={session.title} state={state} act={act}><ConversationName key={session.id} session={session} act={act}/><div className="a-dialog-actions"><button className="a-soft" data-action="session.export" onClick={()=>act('session.export',{id:session.id})}><Download/>Export conversation</button><button className="a-soft a-danger" data-action="session.delete" onClick={()=>open('delete-session')}>Remove chat</button></div></SettingsGroup>}</>}{view.settingsSection==='capabilities'&&<><SettingsGroup id="smart-tools" title="Smart Tools" summary="Add tools and interactive canvas apps" state={state} act={act}><SmartToolsSettings state={state} act={act}/></SettingsGroup><BundleSettings state={state} session={session} act={act}/></>}{view.settingsSection==='maintenance'&&<>{state.runtime?.retention&&<SettingsGroup id="ready-conversations" title="Ready conversations" summary="Idle lifetime and background preparation" state={state} act={act}><WorkerRetentionSettings state={state} act={act}/></SettingsGroup>}<SettingsGroup id="updates" title="Updates" summary={state.updates?.available?`${state.updates.available} updates available`:"App, bundles and module updates"} state={state} act={act}><UpdateSettings state={state} act={act}/></SettingsGroup><MaintenanceSettings state={state} act={act}/></>}</>}
  </section></div>}
 </div>;
}
